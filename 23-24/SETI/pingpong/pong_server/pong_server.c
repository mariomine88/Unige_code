...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &time2))
		fail_errno("Error in get clock time2");
...
/*** TO BE DONE START ***/
	if(clock_gettime(CLOCK_TYPE, &time3))
		fail_errno("Error in get clock time3");
...
/*** TO BE DONE START ***/
	...
	// If accept fails, continue.
	if(request_socket == -1) {
		// If a system call was interrupted (EINTR), try again
		if(errno == EINTR) continue;

		// For other errors, close the server socket and exit with an error message
		close(server_socket);
		fail_errno("Pong Server cannot accept");
	}
	
	//fork if accept succeeds
	if((pid = fork()) == -1)
		fail_errno("failed to fork");
	// in child process, serve the client.
	if(!pid){
...
/*** poRT ***/
	...
	// If getaddrinfo fails, terminate with an error message.
	if((gai_rv = getaddrinfo(NULL, argv[1], &gai_hints, &server_addrinfo)))
		fail(gai_strerror(gai_rv));
	
	// If socket creation fails, terminate with an error message.
	if((server_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)
		fail_errno("error creating socket");

	// If binding the socket fails, terminate with an error message.
	if(bind(server_socket, (struct sockaddr*) server_addrinfo->ai_addr, server_addrinfo->ai_addrlen))
		fail_errno("error binding socket");

	// If listening on the socket fails, terminate with an error message.
