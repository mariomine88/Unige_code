...
and typecheckBlock env =
    function
    | EmptyBlock -> ()
    | Block stmtSeq -> typecheckStmtSeq (enterScope env) stmtSeq
...
let rec evalExp (env: dynamicEnv) =
    function
    | Add(exp1, exp2) -> (evalExp env exp1 |> toInt) + (evalExp env exp2 |> toInt) |> IntValue
    | Mul(exp1, exp2) -> (evalExp env exp1 |> toInt) * (evalExp env exp2 |> toInt) |> IntValue
