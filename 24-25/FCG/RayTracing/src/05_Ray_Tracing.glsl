...
int maxDepth = 10;
int samplesPerPixel = 100; // increse for better quality but slower performance 
...
Sphere spheres[numSpheres] = Sphere[](
    Sphere(vec3(0.0, 1.0, 0.0), 1.0, Material(vec3(0.8, 0.2, 0.2) , vec3(0.84f, 0.83f, 0.02f), 10.0)),
    Sphere(vec3(0.0, -1000.0, 0.0), 1000.0, Material(vec3(0.3725, 0.2902, 0.0745), vec3(0.0), 0.0))
);
...
vec3 Trace(Ray ray,inout uint state) {
    ...
    for (int depth = 0; depth < maxDepth; depth++) {
        ...
        incomingLight += rayColor * hit.material.emissionColour * hit.material.emissionStrength;
