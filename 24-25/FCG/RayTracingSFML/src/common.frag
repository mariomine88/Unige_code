...
void initSpheres() {
    ...
    spheres[0] = Sphere(vec3(0,-1000,0), 1000.0, 
        Material(vec3(0.5), vec3(0), 0.0, 0.0, 0.0, 0.0));
    
    
    //red
    spheres[1] = Sphere(vec3(-5,1,0), 1.0, 
        Material(vec3(1,0,0), vec3(0), 0.0, 0.0, 0.0, 0.0));
    // Yellow reflecting
    spheres[2] = Sphere(vec3(-2.5,1,0), 1.0, 
        Material(vec3(0.5,0.5,0), vec3(0), 0.0, 1.0, 1.0, 0.0));
    // Green light
    spheres[3] = Sphere(vec3(0,1,0), 1.0, 
        Material(vec3(0,1,0), vec3(0,1,0), 10.0, 0.0, 0.0, 0.0));
    // Cyan glass
    spheres[4] = Sphere(vec3(2.5,1,0), 1.0, 
        Material(vec3(0,1,1), vec3(0), 0.0, 0.1, 0.0, 1.5));
    // Blue
    spheres[5] = Sphere(vec3(5,1,0), 1.0, 
        Material(vec3(0,0,1), vec3(0), 0.0, .5, 1.0, 0.0));
}