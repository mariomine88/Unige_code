...
const Materiale m3 = Materiale(GIALLO*0.3,GIALLO*0.7,vec3(0.0),50.0);
const Materiale m_sole = Materiale(vec3(2.0),vec3(2.0),vec3(2.0),0.0);
...
// fonte di luce: (colore > 1)
Sfera sole     = Sfera(vec3(-1.0,2.5,6.0), 0.25, m_sole );
...
vec3 ray_cast( vec2 clip_pos ){
  ...
  vec3 verso_luce = normalize(sole.centro - punto_colpito);
  ...
  // se il raggio secondario non colpisce il sole, il punto è in ombra
  if (dummy.amb.x<=1.0) 
    col *= 0.7;
    // col = mat.amb*luce_ambiente;
...
// animazione
void animate_scene() {
  sole.centro += vec3(5.0*cos(u_time), 0.0, 5.0*sin(u_time));
} 
