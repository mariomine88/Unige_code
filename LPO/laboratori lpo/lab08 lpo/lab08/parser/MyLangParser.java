...
public class MyLangParser implements Parser {
	...
	private ForStmt parseForStmt() throws ParserException{
		...
		consume(OPEN_PAR);
		consume(VAR); 
		final var var = parseVariable().name();
		consume(OF);
	 ...
	 */
	 ...
	 private Exp parseUnary() throws ParserException {
		...
		case FST -> parseFst();
		case SND -> parseSnd();
		default -> parseAtomOrparseDict();
		};
	}

	private Exp parseAtomOrparseDict() throws ParserException {
		var e = parseAtom();
		if (tokenizer.tokenType() == OPEN_DICT)
			return new Dict(e, parseDictAcc());
		return e;
	 }
	...
	*/
	private Exp parseDictAcc() throws ParserException {
		var exp = parseAtom();
	 ...
	 */	
	private Dict parseDictliteral() throws ParserException {
		consume(OPEN_DICT);
