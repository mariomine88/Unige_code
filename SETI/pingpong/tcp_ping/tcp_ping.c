...
/*** TO BE DONE START ***/
	...
	// Convert msg_no to string. On failure, terminate with error.
	if(sprintf(message, "%d", msg_no) < 0)
		fail("Could not copy msg_no into message\n");
...
/*** TO BE DONE START ***/
	...
	// Send message through socket. On failure, terminate with error.
